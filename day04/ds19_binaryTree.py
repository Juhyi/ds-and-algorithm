...
for name in groupList[1:]: #레드베벳부터 끝까지
    node =TreeNode()
    node.data = name
    ...
    while True:
        if name < curr.data: #왼쪽 트리로
            if curr.left == None: #왼쪽에 트리가 구성되었지않으면 
                curr.left = node
                break # while문 탈출
            else:
                curr = curr.left
...
while True:
    if deleteName == curr.data: # 삭제할 데이터 찾았으면
        if curr.left == None and curr.right == None: # 리프노드라 삭제 완전 쉬움
            if parent.left == curr: # 내가 부모의 왼족에 붙어있으면
                parent.left = None
            else: # 내가 부모의 오른쪽에 붙어있으면
                parent.right = None
            
            del(curr) # 연결이 끊어지 노드를 완전히 삭제
        elif curr.left != None and curr.right ==None: # 내 노드 왼쪽에 지식노드가 있으면
